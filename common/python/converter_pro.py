...
class ProxyConverter:
    ...
    def parse_url_based(link):
        try:
            ...
            if scheme in ["vless", "trojan"]:
                ...
                # 修正2: SNI字段应为 sni (OpenClash/Meta 偏好 sni)
                node["sni"] = params.get("sni", u.hostname)
                node["skip-cert-verify"] = True if params.get("insecure") == "1" else False
                
                # 修正3: 添加 fingerprint (Meta 特有)
                if params.get("fp"):
                    node["fingerprint"] = params.get("fp")

                # 修正4: 处理 obfs (格式为 obfs: salamander)
                if params.get("obfs") and params.get("obfs") != "none":
                    node["obfs"] = params.get("obfs")
                    node["obfs-password"] = params.get("obfs-password", "")
                
                # 修正5: 自动转换带宽参数 (Meta 需要 up/down)
                if params.get("up_mbps"): 
                    node["up"] = f"{params.get('up_mbps')} Mbps"
                if params.get("down_mbps"): 
                    node["down"] = f"{params.get('down_mbps')} Mbps"
    ...
    def generate_simple_config(proxies):
        ...
        # 修正6: 更新字段写入顺序，包含 password, sni, up, down, fingerprint
        key_order = [
            "name", "type", "server", "port", "uuid", "password", "cipher",
            "tls", "servername", "sni", "skip-cert-verify", "network", "flow", 
            "client-fingerprint", "fingerprint", "udp", 
            "up", "down", # Hysteria 2 带宽
            "ws-opts", "grpc-opts", "h2-opts", "reality-opts", 
            "obfs", "obfs-password", # Hysteria 2 混淆
