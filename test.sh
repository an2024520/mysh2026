    ...
    while true; do
        ...
        case "$hop_choice" in
            ...
            2)
                ...
                iptables -t nat -D PREROUTING -p udp --dport "$d_start":"$end_port" -j REDIRECT 2>/dev/null
...
manage_swap() {
    # 检查是否为 OpenVZ
    if [[ -d "/proc/vz" ]]; then
        echo -e "${RED}错误: 您的 VPS 基于 OpenVZ，不支持添加 Swap！${PLAIN}"
        read -p "按回车继续..." 
        return
    fi
    ...
    while true; do
        clear
        echo -e "${BLUE}========= Swap 虚拟内存管理 =========${PLAIN}"
        echo -e "当前 Swap 状态:"
        swapon --show | cat || echo -e "${YELLOW}无 Swap${PLAIN}"
        echo ""
        echo -e " ${GREEN}1.${PLAIN} 添加 Swap (建议设置为内存的 2 倍)"
        ...
        case "$swap_choice" in
            1)
                echo -e "${YELLOW}请输入需要添加的 Swap 值 (单位 MB，例如 4096): ${PLAIN}"
                read -p "Swap 大小: " swapsize
                ...
                if grep -q "swap" /etc/fstab; then
                    echo -e "${RED}错误: 已存在 Swap 配置，请先删除后再添加！${PLAIN}"
                    read -p "按回车继续..." 
                    continue
                fi

                echo -e "${YELLOW}正在创建 ${swapsize} MB 的 Swap 文件...${PLAIN}"
                dd if=/dev/zero of=/swapfile bs=1M count=${swapsize}
                chmod 600 /swapfile
                mkswap /swapfile
                swapon /swapfile
                echo '/swapfile none swap defaults 0 0' >> /etc/fstab
                echo -e "${GREEN}✅ Swap 添加成功！当前信息:${PLAIN}"
                swapon --show
                read -p "按回车继续..." 
                ;;
            2)
                if grep -q "swap" /etc/fstab; then
                    echo -e "${YELLOW}正在删除现有 Swap...${PLAIN}"
                    sed -i '/swap/d' /etc/fstab
                    echo "3" > /proc/sys/vm/drop_caches
                    swapoff -a
                    rm -f /swapfile
                    echo -e "${GREEN}✅ Swap 已成功删除！${PLAIN}"
                else
                    echo -e "${RED}未检测到现有 Swap，无法删除。${PLAIN}"
                fi
                read -p "按回车继续..." 
                ;;
            0) return ;;
...
show_menu() {
    while true; do
        ...
        echo -e " ${SKYBLUE}6.${PLAIN} Swap 虚拟内存管理 ${YELLOW}(添加/删除)${PLAIN}"
        echo -e " --------------------------------------------"
        echo -e " ${GREEN}7.${PLAIN} UDP 端口跳跃管理 ${YELLOW}(适配 Sing-box Hy2)${PLAIN}"
        echo -e " ${GREEN}8.${PLAIN} SSH 安全配置 ${YELLOW}(导入公钥/禁用密码)${PLAIN}"
        echo -e " --------------------------------------------"
        echo -e " ${GRAY}0. 返回上一级${PLAIN}"
        echo ""
        read -p "请选择操作: " choice
