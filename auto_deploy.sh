#!/bin/bash
# ============================================================
#  Commander Auto-Deploy (v9.0 System-Wide Fix)
#  - 架构: 引入全局系统初始化 (时间同步 + 智能分流)
#  - 修复: 纯 IPv6 环境下的优先级黑洞问题 (/etc/gai.conf)
# ============================================================
...
system_init_check() {
    ...
    # --- 2. 网络环境检测与分支处理 (Safe Branching) ---
    echo -e "${YELLOW}   > 正在检测网络连通性 (Strict Mode)...${PLAIN}"
    
    # [安全卫语句] 如果 IPv4 通畅，绝对不碰 gai.conf
    ...
    # B. [修复] 强制系统级 IPv6 优先 (/etc/gai.conf)
...
deploy_logic() {
    ...
    echo -e "\n${YELLOW}======================================================${PLAIN}"
    echo -e "${YELLOW} [重要提示] Cloudflare Tunnel 配置指南${PLAIN}"
    echo -e "${YELLOW}======================================================${PLAIN}"
    echo -e " 如果您在 Cloudflare 网页端配置 Public Hostname："
    echo -e " 1. Service 类型选择: ${GREEN}HTTP${PLAIN}"
    echo -e " 2. URL 必须填写:     ${GREEN}127.0.0.1:端口${PLAIN} (例如 127.0.0.1:34333)"
    echo -e " ${RED}* 切勿使用 localhost，否则会导致 IPv6 绑定错误！${PLAIN}"
    echo -e "======================================================\n"
...
show_dashboard() {
    ...
    if [[ "$INSTALL_SB" == "true" ]]; then
        ...
        if [[ "$DEPLOY_SB_HY2" == "true" ]]; then
            if [[ -n "$VAR_SB_HY2_DOMAIN" ]]; then echo -e "  └─ Hysteria 2      [ACME: ${GREEN}${VAR_SB_HY2_DOMAIN}${PLAIN}]"; else echo -e "  └─ Hysteria 2      [Self: ${GREEN}${VAR_SB_HY2_PORT}${PLAIN}]"; fi
        fi
        has_item=true
    fi
    if [[ "$INSTALL_XRAY" == "true" ]]; then
    ...
    if [[ "$has_item" == "false" ]]; then echo -e "${GRAY}  (购物车是空的, 请选择商品...)${PLAIN}"; fi
    echo -e "=============================================="
}
...
menu_protocols() {
    while true; do
        ...
        case $c in
            ...
            5) if [[ "$DEPLOY_SB_HY2" == "true" ]]; then DEPLOY_SB_HY2=false; unset VAR_SB_HY2_PORT VAR_SB_HY2_DOMAIN; else DEPLOY_SB_HY2=true; INSTALL_SB=true; echo -e "${YELLOW}[Hysteria 2 设置]${PLAIN}"; read -p "  域名 (Optional): " d; VAR_SB_HY2_DOMAIN=$d; if [[ -n "$VAR_SB_HY2_DOMAIN" ]]; then read -p "  端口 (443): " p; VAR_SB_HY2_PORT=${p:-443}; else read -p "  UDP端口 (10086): " p; VAR_SB_HY2_PORT=${p:-10086}; fi; fi ;;
...
menu_warp() {
    while true; do
        clear; echo -e "${SKYBLUE}=== WARP 路由配置 ===${PLAIN}"; echo -e "当前状态: [$(get_status $INSTALL_WARP)]"; echo ""; echo -e " 1. 启用/配置 WARP 账号"; echo -e " 2. 选择分流模式"; echo -e " 3. ${RED}清空 WARP 购物车 (Remove)${PLAIN}"; echo ""; echo -e " 0. 返回"
        read -p "选择: " w
        case $w in
            1) INSTALL_WARP=true; [[ -z "$WARP_MODE_SELECT" ]] && WARP_MODE_SELECT=5; echo -e "   1. 自动注册 (默认)"; echo -e "   2. 手动录入"; read -p "   选择: " acc; if [[ "$acc" == "2" ]]; then read -p "   Private Key: " k; export WARP_PRIV_KEY="$k"; read -p "   IPv6 Address: " i; export WARP_IPV6="$i"; read -p "   Reserved: " r; export WARP_RESERVED="$r"; else unset WARP_PRIV_KEY WARP_IPV6 WARP_RESERVED; fi ;;
            ...
            3) INSTALL_WARP=false; unset WARP_MODE_SELECT WARP_PRIV_KEY WARP_IPV6 WARP_RESERVED; echo -e "${YELLOW}已从购物车移除 WARP。${PLAIN}"; sleep 1 ;;
            0) break ;;
        esac
    done
}
...
menu_argo() {
    while true; do
        clear; echo -e "${SKYBLUE}=== Argo 配置 ===${PLAIN}"; echo -e " 1. 启用 Argo"; echo -e " 2. 清空 Argo"; echo " 0. 返回"
        read -p "选择: " c
        case $c in
...
while true; do
    ...
    read -p "选项: " m
    case $m in
        1) menu_protocols ;; 2) menu_warp ;; 3) menu_argo ;;
        0) if [[ "$INSTALL_SB" != "true" ]] && [[ "$INSTALL_XRAY" != "true" ]] && [[ "$INSTALL_ARGO" != "true" ]]; then echo -e "${RED}购物车是空的！请先选择商品。${PLAIN}"; sleep 2; else deploy_logic; break; fi ;;
    esac
done
